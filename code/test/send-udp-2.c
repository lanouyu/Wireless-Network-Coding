...
uint32_t encode(uint32_t mysize, u_char * payload) {
...
//===========================================================================
	...
	while (1) {
		...
		payload_s = encode(50, (u_char *)payload); /* 计算负载内容的长度 */
		printf("\n254:payload_s %x\n\n",payload_s );
...
int udp_2() {
    ...
    libnet_t *handle; /* Libnet句柄 */
    int packet_size; /* 构造的数据包大小 */
    char *device = "ens38"; /* 设备名字,也支持点十进制的IP地址,会自己找到匹配的设备 */
    char *src_ip_str = "192.168.1.114"; /* 源IP地址字符串 */
    char *dst_ip_str = "192.168.1.255"; /* 目的IP地址字符串 */
    //char *dst_ip_str = "192.168.0.133";
    //u_char src_mac[6] = {0x00, 0x0c, 0x29, 0x2b, 0xaa, 0xd2}; /* 源MAC */
    u_char src_mac[6] = {0x00, 0x0c, 0x29, 0xbf, 0xad, 0x3f}; /* 目的MAC */
...
int main(int argc, char** argv) {
  ...
  int err;
  void *tret;
  err=pthread_create(&tid1,NULL,udp_1,NULL);//创建线程
  if(err!=0)
  {
    printf("pthread_create error:%s\n",strerror(err));
    exit(-1);
  }
  err=pthread_create(&tid2,NULL,udp_2,NULL);
  if(err!=0)
  {
    printf("pthread_create error:%s\n",strerror(err));
     exit(-1);
